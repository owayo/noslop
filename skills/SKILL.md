...
description: >-
  ...
  (数字や固有名詞が消えていないか・同じ直しを一律に当てていないか) の場面で発動。コードに書いた
  日本語のコメント (noslop check にコードのファイルを渡すとコメントだけを検査する) や、gws で
  Google ドキュメント・スプレッドシートに書き込む文章の見直し、noslop のフックが指摘を返した・
  gws の書き込みを止めたときの対応にも使う。
...
- コードに日本語のコメントを書いた・直したとき (`noslop check src/lib.rs` のようにコードのファイルを渡すと、コメントだけを検査する。文字列は読まない)

英語の文章には使わない (英語の段落は数えない)。
...
noslop のフックは、ファイルを書いた直後・応答を終えたとき (コミットしていない変更)・gws で Google ドキュメントやスプレッドシートに書き込む前に、短い改稿指示を返す。どれも上の手順 2 と同じく、効果の大きい箇所だけを直し、文脈上必要な箇所は理由を報告に書いて残す。

- **gws の書き込みが止められたとき** (「この書き込みを止めました。まだ書き込んでいません」): 書き込む文章を直してから、gws のコマンドを打ち直す。直さずに書き込むと決めたら、同じコマンドをそのままもう一度実行する (同じ内容の打ち直しは 30 分以内に 1 回だけ通る)。止めた理由と残した判断は、書き手への報告に添える
