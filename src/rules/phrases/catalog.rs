...
const P02_ENTRIES: &[Entry] = &[
    ...
    Entry::exp_re(r"本(?:報告書|レポート)(?:では|は)", Info),
...
const P06_ENTRIES: &[Entry] = &[
    ...
    // 「〜なのは、」で要点を先回りして予告する形。「大切」「重要」は校正済みの項目があるので
    // 含めない (同じ位置から始まる長い一致が優先され、校正済みの警告を上書きしてしまうため)。
    // 読点までを条件にして、「大事なのは分かる」のような別の使い方を拾わない
    Entry::exp_re(
        r"(?:大事|肝心|必要|厄介|危険|問題|意外|面倒|印象的|特徴的|理想的|効果的|注意すべき|見落としがち)なのは、",
        Info,
    ),
    // 「〜ことが大事です」は人の文章のほうに多かったので入れない
    Entry::exp_re(r"ことが大切(?:です|だ|になります)", Info),
];
...
「非常に重要」「極めて重要」「大切なのは」「言うまでもなく」のように、重要さを語で言い立てる表現を探します。実験的な項目として、「大事なのは、」「厄介なのは、」「注意すべきなのは、」のように評価の語で要点を先回りして予告する「〜なのは、」の形と、「〜ことが大切です」も探します。
...
const P08_ENTRIES: &[Entry] = &[
    ...
    // 「鍵を握りしめる」のような文字どおりの意味を拾わないよう、終止形・連体形だけにする
    Entry::exp_re(r"(?:鍵|カギ)を握る", Info),
...
「核心的」「鍵となる」「根本的な」「多角的」「包括的」「総合的」「不可欠」のように、主張の中身を言わずに重要さや網羅の感触だけを足す形容を探します。実験的な項目として、「〜の鍵になる」「カギを握る」「〜の秘訣」「〜への第一歩」も探します。
...
既定で有効な語句は、人間とAIのコーパスで確かめたものです。「不可欠」は人間の文章にも一定数出たため、情報に下げています。「鍵になる」「カギを握る」「秘訣」「第一歩」は、複数のモデル (Claude・GPT・Gemini) に書かせた文章と生成 AI の普及前に人が書いた文章を比べた下調べで、生成側に多く出ました。ただし noslop のコーパスでは校正していないので、未校正の実験的な項目として情報で扱います。「第一歩」は式辞や挨拶の文でも人が使います。",
    },
    entries: P08_ENTRIES,
...
// 数字の前後の空白 (「次の 4 つ」) も同じに扱う
const P11_ENTRIES: &[Entry] = &[
    Entry::exp_re(
        r"(?:理由|ポイント|要因|メリット|デメリット|課題|方法|特徴|軸|要素|観点|注意点|ところ|こと)(?:は|が)(?:主に|大きく)?\s?[0-9０-９一二三四五六七八九十]+\s?つ(?:あります|ありました|です|ある|だ)",
        Info,
    ),
    Entry::exp_re(r"大きく分けて\s?[0-9０-９一二三四五六七八九十]+\s?つ", Info),
    Entry::exp_re(
        r"(?:以下|次)の\s?[0-9０-９一二三四五六七八九十]+\s?(?:つ|点)",
        Info,
    ),
    Entry::exp_re(r"[0-9０-９一二三四五六七八九十]+\s?(?:つ|点)を挙げ", Info),
];
...
「理由は3つあります」「ポイントは主に2つです」「大きく分けて3つ」「以下の3点」「次の4つ」「注意点が2つあります」「3つを挙げます」のように、これから並べる項目の数を先に宣言する書き出しを探します。「次の 4 つ」のように数字の前後に空白を入れた書き方も同じに扱います。
...
手癖として知られている型ですが、コーパスでの誤検知率は測っていません。実験的なルールとして `--experimental` か設定で有効にしたときだけ動きます。「次の4つ」「〜が2つあります」「3つを挙げます」は、複数のモデルに書かせた文章と人の文章を比べた下調べで生成側に多く出た形です。ただし「対応内容は次の2つです」のように人も普通に書く形なので、誤検知率を測るまでは実験的なルールのままにします。",
    },
    entries: P11_ENTRIES,
...
mod tests {
    ...
    fn p02_experimental_report_prefaces() {
        let md = "本報告書では、移行の手順をまとめる。本レポートは三部構成である。\n";
        assert!(run(&rule(&P02), md).is_empty());
        let d = run_with(&rule(&P02), md, experimental());
        assert_eq!(matched(md, &d), vec!["本報告書では", "本レポートは"]);
        assert!(
            d.iter()
                .all(|d| d.status == RuleStatus::Experimental && d.severity == Severity::Info)
        );
        // 「本」の付かない報告書は拾わない
        let md = "前回の報告書では、移行の手順を扱った。\n";
        assert!(run_with(&rule(&P02), md, experimental()).is_empty());
    }
    ...
    fn p06_pseudo_clefts_ignore_other_uses() {
        // 読点の続かない「〜なのは」、疑問の「〜なのか」、人の文章に多い「〜ことが大事です」は拾わない
        for md in [
            "それが大事なのは分かっている。\n",
            "どれが必要なのかを先に決める。\n",
            "毎日続けることが大事です。\n",
    ...
    fn p08_experimental_key_and_secret_phrases() {
        let md = "記録の頻度が継続の鍵になります。長続きの秘訣は記録を減らすことだ。\
                  成否のカギを握るのは準備だ。まずは改善への第一歩を踏み出す。\n";
        assert!(run(&rule(&P08), md).is_empty());
        let d = run_with(&rule(&P08), md, experimental());
        assert_eq!(
            matched(md, &d),
            vec!["鍵になり", "秘訣", "カギを握る", "第一歩"]
        );
        assert!(
            d.iter()
                .all(|d| d.status == RuleStatus::Experimental && d.severity == Severity::Info)
