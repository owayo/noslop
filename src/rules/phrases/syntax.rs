...
人間とAIのコーパスで確かめた構文で、重大度は情報です。辞書を使わず、述語は活用の語幹 (「示し」「もたらさ」など) で照合します。未然形の「もたらさ」「示さ」「生み出さ」(「何も生み出さない」) も拾います。「指示」「表示」のように前に漢字が付く「示」は動詞とみなしません。「暗示する」は人の随筆にも出る語で、誤検知率を測っていないため実験的にしています。",
};
